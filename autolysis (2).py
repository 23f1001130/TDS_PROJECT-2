...
def create_visuals(correlation, outliers, data, output_dir):
    print("Creating visualizations...")  # Debug
    ...
    # 2. Outlier Detection Plot (Example: Boxplot for each numerical column)
    outliers_path = os.path.join(output_dir, 'outlier_plot.png')
    ...
    # 3. Distribution Plots (Histograms for each numerical column)
    dist_path = os.path.join(output_dir, 'distribution_plot.png')
    numeric_data = data.select_dtypes(include=[np.number])
    plt.figure(figsize=(10, 6))
    for col in numeric_data.columns:
        sns.histplot(numeric_data[col], kde=True, label=col)
    plt.legend()
    ...
    print("Visualizations created.")  # Debug
